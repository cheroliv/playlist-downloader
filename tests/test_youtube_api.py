...
def test_create_playlist_success(mocker, caplog):
    ...
    result = create_playlist(mock_credentials, "Test Title", "Test Description", private=True)
...
# Scenario 7: Failure to get URL (playlist not found)
def test_get_playlist_url_not_found(mocker, caplog):
    """
    Checks that the function returns a Left if the playlist does not exist.
    LDD: Verifies error logs.
    """
    mock_credentials = MagicMock()
    mock_credentials.universe_domain = "googleapis.com"
    ...
    assert "Failed to retrieve URL" in caplog.text
