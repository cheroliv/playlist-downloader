...
We welcome contributions! To ensure code quality and maintainability, we adhere to the following principles:

* **Functional Style**: We favor pure functions and immutability.
* **TDD/LDD**: Tests are written before production code. We validate not only return values but also logs.
* **Static Typing**: We use `mypy` to ensure type safety.
* **Linting & Formatting**: We use `ruff` to maintain a consistent code style.
